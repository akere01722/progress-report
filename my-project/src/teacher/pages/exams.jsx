...
export default function TeacherExams() {
  ...
  // ✅ placeholders (so dropdown shows "Select ..." first)
  ...
  const submit = async () => {
    setSaving(true);
    try {
      // Replace this with Supabase insert/update later
    ...
    <div className="w-full">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-gray-500 mt-1">
            Select level, semester and course to enter exam scores.
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
        ...
        <p className="mt-3 text-sm text-gray-500">
          {canShowTable
            ? `Now entering exam marks for ${level} • ${semester} • ${course}`
          ...
          <div className="px-5 sm:px-6 py-4 border-t border-gray-100 flex justify-end">
            <button
              ...
              className={`inline-flex items-center justify-center gap-2 rounded-xl px-5 py-3
                ...
                ${saving ? "bg-green-400" : "bg-green-600 hover:bg-green-700 hover:-translate-y-0.5"}`}
              type="button"
            >
              <FiUpload />
