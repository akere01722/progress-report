...
/**
 * StudentExamReport.jsx
 * ✅ Same style + modifications as CA report:
 *  - Semester dropdown (FIRST / SECOND)
 *  - NO print button
 *  - NO hide details
 *  - NO teacher/admin signature blocks
 *  - Clean + classic + responsive
 *  - Download button (JSON for now)
 *
 * Exam is /70 per course (adjust if yours is different)
 */
...
export default function StudentExamReport() {
    ...
    const fetchExamData = async () => {
      ...
      if (user) {
        ...
        if (studentData) {
          ...
          if (examData && !error) {
            // Group by semester
            const grouped = examData.reduce((acc, record) => {
  ...
  // --------------------------
  const total = useMemo(
    () => report.courses.reduce((sum, c) => sum + (Number(c.exam) || 0), 0),
    [report.courses]
  );
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `EXAM_Report_${report.student.matricule}_${report.semester}.json`;
    a.click();
    URL.revokeObjectURL(url);
  };

  // simple remark per course (optional)
  const courseRemark = (score, max) => {
    const pct = (Number(score) / Number(max || 1)) * 100;
    if (pct >= 70) return { text: "Excellent", cls: "bg-green-100 text-green-700" };
    if (pct >= 50) return { text: "Good", cls: "bg-blue-100 text-blue-700" };
    return { text: "Needs Work", cls: "bg-red-100 text-red-700" };
  };
        ...
        <div className="p-5 sm:p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="lg:col-span-2 rounded-2xl border border-gray-100 bg-gray-50 p-5 sm:p-6">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Details used for this exam report.
              </p>
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-5 sm:p-6 shadow-sm">
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                ...
                <MiniStat label="Avg" value={`${avg}/70`} />
              </div>
          ...
          <div className="mt-6 bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <div className="p-5 border-b border-gray-100">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Each exam score is recorded out of 70.
              </p>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {report.courses.map((c, idx) => {
                    const score = Number(c.exam) || 0;
                    const r = courseRemark(score, c.max);
                    return (
                      <tr key={c.code} className="border-t hover:bg-gray-50">
                        ...
                        <td className="px-5 py-3 text-center">
                          <span
                            className={`inline-flex rounded-full px-3 py-1 text-xs font-bold ${r.cls}`}
                          >
                            {r.text}
                          </span>
                        </td>
