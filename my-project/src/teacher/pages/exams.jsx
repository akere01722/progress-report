...
const EXAM_DRAFTS_KEY = "teacher_exam_drafts";
...
const writeDrafts = (drafts) => {
  localStorage.setItem(EXAM_DRAFTS_KEY, JSON.stringify(drafts));
};
...
export default function TeacherExams() {
  ...
  const currentSubmission = useMemo(() => {
    ...
    return (
      submissions.find(
        (submission) =>
          submission.assessmentType === "EXAM" &&
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.success("Exam marks submitted. Waiting for admin approval.");
      setDraftMessage("Submission sent to admin for approval.");
    } catch (error) {
      toast.error(error?.message || "Failed to submit exam marks.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">
            Submit Examination Marks
          </h1>
          <p className="text-gray-500 mt-1">
            Select year, semester, class and course, then submit exam marks for admin approval.
          </p>
        </div>
      </div>
      ...
      {canShowTable && (
        <div className="mt-6 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  <tr key={student.matricule} className="border-t hover:bg-gray-50 transition">
                    ...
                    <td className="px-5 sm:px-6 py-3 text-center">
                      <input
                        ...
                        className="w-24 rounded-xl border border-gray-200 bg-white px-3 py-2 text-center focus:outline-none focus:ring-2 focus:ring-green-500"
                        placeholder="0-70"
                      />
                    </td>
                  </tr>
          ...
          <div className="px-5 sm:px-6 py-4 border-t border-gray-100 flex flex-wrap justify-end gap-3">
            ...
            >
              <FiUpload />
              {saving ? "Submitting..." : "Submit Exam Marks"}
            </button>
          </div>
        </div>
      )}
